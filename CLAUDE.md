...
- **Do not deploy to production without explicit user approval.** Always wait for the user to say "deploy" or "deploy to production" before running `wrangler pages deploy`. Preview/branch deployments for testing are fine without approval, but production deploys require an explicit go-ahead.
